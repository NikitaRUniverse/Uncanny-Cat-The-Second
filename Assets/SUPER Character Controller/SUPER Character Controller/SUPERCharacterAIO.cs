            ...
            switch (currentGroundMovementSpeed){
                case GroundSpeedProfiles.Walking:{
                    ...
                    if((canCrouch&&crouchInput_FrameOf)||crouchOverride){
                        ...
                        StartCoroutine(ApplyStance(stanceTransitionSpeed,Stances.Crouching));
                        break;
                    }else if(isSprintAllowed && (canSprint&&sprintInput_FrameOf && ((enableStaminaSystem && jumpingDepletesStamina)? currentStaminaLevel>s_minimumStaminaToSprint : true) && (enableSurvivalStats ? (!currentSurvivalStats.isDehydrated && !currentSurvivalStats.isStarving) : true))||sprintOverride){
                        isCrouching = false;
                        isSprinting = true;
                ...
                case GroundSpeedProfiles.Crouching:{
                    ...
                    if((toggleCrouch ? crouchInput_FrameOf : !crouchInput_Momentary)&&!crouchOverride && OverheadCheck()){
                        ...
                        StartCoroutine(ApplyStance(stanceTransitionSpeed,Stances.Standing));
                        break;
                    }else if(isSprintAllowed && ((canSprint && sprintInput_FrameOf && ((enableStaminaSystem && isSprintAllowed && jumpingDepletesStamina)? currentStaminaLevel>s_minimumStaminaToSprint : true)&&(enableSurvivalStats ? (!currentSurvivalStats.isDehydrated && !currentSurvivalStats.isStarving) : true))||sprintOverride) && OverheadCheck()){
                        isCrouching = false;
                        isSprinting = true;
                        currentGroundSpeed = sprintingSpeed;
                        currentGroundMovementSpeed = GroundSpeedProfiles.Sprinting;
